...
pub enum InnerError {
    ...
    #[snafu(display("{:?} call failed. '{}' returned a {} result.", api, url, status))]
    CallFailed { api: API, url: String, status: u16 },

    #[snafu(display("{:?} chart failed to load {} - {}.", api, code, description))]
    ChartFailed { api: API, code: String, description: String },
    ...
    #[snafu(display("Unexpected {:?} failure. '{}' returned a {}", api, url, code))]
