...
use crate::exchange::{error::Result, error, fetch};
...
const YAHOO_BASE_URL: &'static str = "https://query1.finance.yahoo.com/v8/finance/chart/";
...
async fn load(url: &Url) -> Result<Data> {
    ...
    let response = reqwest::get(url.clone()).await.context(error::RequestFailed { api: Yahoo })?;
    ensure!(
      response.status().is_success(),
      error::CallFailed{ api: Yahoo, url: response.url().to_string(), status: response.status().as_u16() }
   );

    let data = response.text().await.context(error::UnexpectedErrorRead { url: url.to_string() })?;
    let chart = serde_json::from_str::<YahooResponse>(&data).context(error::BadData { api: Yahoo })?.chart;
    ...
    if !chart.result.is_some() {
        ...
        error::ChartFailed { api: Yahoo, code: err.code, description: err.description }.fail()?;
    }

    // we have a result to process
    let result = chart.result.context(error::UnexpectedError { api: Yahoo })?;
    ensure!(result.len() > 0, error::UnexpectedError { api: Yahoo });
    Ok(result[0].clone())
}
...
fn aggregate_bars(data: Data) -> Result<Vec<Bar>> {
    ...
    ensure!(!timestamps.is_empty(), error::MissingData { api: Yahoo, reason: "no timestamps for OHLCV data" });
    ensure!(!quotes.is_empty(), error::MissingData { api: Yahoo, reason: "no OHLCV data" });

    // make sure timestamps lines up with the OHLCV data
    let quote = &quotes[0];
    ensure!(timestamps.len() == quote.volumes.len(), error::MissingData { api: Yahoo, reason: "timestamps do not line up with OHLCV data" });
    ensure!(timestamps.len() == quote.opens.len(), error::MissingData { api: Yahoo, reason: "'open' values do not line up the timestamps" });
    ensure!(timestamps.len() == quote.highs.len(), error::MissingData { api: Yahoo, reason: "'high' values do not line up the timestamps" });
    ensure!(timestamps.len() == quote.lows.len(), error::MissingData { api: Yahoo, reason: "'low' values do not line up the timestamps" });
    ensure!(timestamps.len() == quote.closes.len(), error::MissingData { api: Yahoo, reason: "'close' values do not line up the timestamps" });
...
pub async fn return_chart_data(symbol: &str, range: ChartRange, interval: CandlestickInterval) -> Result<Vec<Bar>> {
